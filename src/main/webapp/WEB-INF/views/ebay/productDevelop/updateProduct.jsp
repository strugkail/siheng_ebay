...
<head>
    ...
    <style type="text/css">
        /*#imgPreview:hover {*/
            /*transform: scale(3.1);*/
            /*-webkit-transform: scale(3.1); !*Safari 和 Chrome*!*/
            /*-moz-transform: scale(3.1); !*Firefox*!*/
            /*-ms-transform: scale(3.1); !*IE9*!*/
    ...
    <script type="text/javascript">
        ...
        function processCodeManagerSource(data){
            ...
            var content;
            var row;
            if(typeof(data[0].publishPrice)=="undefined"){
                for(var i=0; i<data.length; i++){
                    row =
                        ...
                        + '<div class="input-group"><input class="form-control" type="number" min="0" id="quantity_'+i+'" name="purchaseConfigInfoList['+i+'].quantity" value="'+data[i].quantity+'"/>'
                        + '<span onclick="refresh('+i+',\'quantity_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><span class="input-group-addon">'+currency+'</span><input class="form-control" type="number" min="0" step="0.01" id="publishPrice_'+i+'" name="purchaseConfigInfoList['+i+'].publishPrice" placeholder="12" value="'+sellingPrice+'"/>'
                        + '<span onclick="refresh('+i+',\'publishPrice_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><span class="input-group-addon">'+currency+'</span><input class="form-control" type="number" min="0" step="0.01" id="publishTransPrice_'+i+'" name="purchaseConfigInfoList['+i+'].publishTransPrice" value="'+freight+'" readonly/>'
                        ...
                        + '<th><div class="input-group"><span class="input-group-addon">'+currency+'</span><input class="form-control" type="number" min="0" step="0.01" id="profit_'+i+'" name="purchaseConfigInfoList['+i+'].profit" value="'+profit+'"/>'
                        + '<span onclick="refresh('+i+',\'profit_\')" class="input-group-addon">Λ</span></div></th>'
                ...
                for(var i=0; i<data.length; i++){
                    row =
                        ...
                        + '<th><select class="form-control select" id="sourceId_'+i+'" name="purchaseConfigInfoList['+i+'].sourceId" style="width: 200px;"></select></th>'
                        + '<th><input type="hidden" name="" value="'+data[i].sourceId+'"/>'
                        + '<div class="input-group"><input class="form-control" type="number" min="0" id="quantity_'+i+'" name="purchaseConfigInfoList['+i+'].quantity" placeholder="'+data[i].quantity+'" value="'+data[i].quantity+'"/>'
                        + '<span onclick="refresh('+i+',\'quantity_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><span class="input-group-addon">'+currency+'</span><input class="form-control" type="number" min="0" step="0.01" id="publishPrice_'+i+'" name="purchaseConfigInfoList['+i+'].publishPrice" value="'+data[i].publishPrice+'"/>'
                        + '<span onclick="refresh('+i+',\'publishPrice_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><span class="input-group-addon">'+currency+'</span><input class="form-control" type="number" min="0" step="0.01" id="publishTransPrice_'+i+'" name="purchaseConfigInfoList['+i+'].publishTransPrice" value="'+data[i].publishTransPrice+'" readonly/>'
                        + '<span onclick="refresh('+i+',\'publishTransPrice_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><input class="form-control" type="number" min="0" step="0.01" id="costPrice_'+i+'" name="purchaseConfigInfoList['+i+'].costPrice" value="'+data[i].costPrice+'"/>'
                        + '<span onclick="refresh('+i+',\'costPrice_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><input class="form-control" type="number" min="0" step="0.1" id="weight_'+i+'" name="purchaseConfigInfoList['+i+'].weight" value="'+data[i].weight+'"/>'
                        + '<span class="input-group-addon">g</span>'
                        + '<span onclick="refresh('+i+',\'weight_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><span class="input-group-addon">'+currency+'</span><input class="form-control" type="number" min="0" step="0.01" id="profit_'+i+'"  name="purchaseConfigInfoList['+i+'].profit" value="'+data[i].profit+'"/>'
                        + '<span onclick="refresh('+i+',\'profit_\')" class="input-group-addon">Λ</span></div></th>'
                        + '<th><div class="input-group"><input class="form-control" type="text" id="profitRate_'+i+'" name="purchaseConfigInfoList['+i+'].profitRate" value="'+profitMargin+'" /><span class="input-group-addon">%</span></div></th>'
                        + '<th><button class="btn btn-danger" type="button" onclick="dele_purchaseConfigInfo('+i+');">删除</button></th>'
                        + '</tr>';
...
//                debug:true, //调试模式，即使验证成功也不会跳转到目标页面
    ...
    $(function () {
        if($("#saleTypeId").val()>2){
            $("#sellingPrice").removeAttr("readonly");
            $("#freight").removeAttr("readonly");
            $("#name").removeAttr("readonly");
            $("#costPrice").removeAttr("readonly");
            $("#declaredValue").removeAttr("readonly");
            $("#defaultWeight").removeAttr("readonly");
            $("#profitMargin").removeAttr("readonly");
            $("#categoryName_div").remove();
        }
...
<ul class="nav nav-tabs">
    <li role="presentation" class="active"><a href="#">产品修改页</a></li>
</ul>
<div class="container-fluid">
    <form:form class="" id="inputForm" modelAttribute="product" action="${ctx}/product/develop/update">
        ...
        <input type="hidden" id="profit" name="profit" value="${product.profit}"/>
        ...
        <input type="hidden" id="saleTypeId" name="saleTypeId" value="${product.developmentType}"/>
        ...
        <div class="row">
            ...
            <label>${site.siteShortName}</label>
            <input  name="siteShortName" type="hidden" value="${site.siteShortName}" />
        </div>
        <hr/>
        <div class="row">
            ...
            <div class="col-md-3">
                <c:if test="${product.developmentType==1}" var="type">
                    <input class="form-control" id="developmentType" type="text" name="developmentTypes" value="	海外仓精品" readonly placeholder="" >
                </c:if>
                <c:if test="${product.developmentType==2}" var="type">
                    <input class="form-control" id="developmentType" type="text" name="developmentTypes" value="虚拟海外仓精品" readonly placeholder="" >
                </c:if>
                <c:if test="${product.developmentType==3}" var="type">
                    <input class="form-control" id="developmentType"  type="text" name="developmentTypes" value="	中国直发仓铺货" readonly placeholder="" >
                </c:if>
                <c:if test="${product.developmentType==4}" var="type">
                    <input class="form-control" id="developmentType" type="text" name="developmentTypes" value="	虚拟海外仓铺货" readonly placeholder="" >
                </c:if>
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <input class="form-control" id="name" name="name" type="text" readonly value="${product.name}" placeholder="">
            </div>
        </div>
        <hr/>
        ...
        <div class="row">
            ...
            <div class="col-md-11">
                <input class="form-control" id="productUrl" name="productUrl" type="text" value="${product.productUrl}" readonly  placeholder="">
            </div>
        </div>
        <hr/>

        <div class="row" id="categoryName_div">
            <label class="col-md-1 control-label text-right" for="categoryName">刊登分类1：</label>
            <div class="col-md-3">
                <input class="form-control" id="categoryName" name="categoryName" type="text" value="${calculateDto.categoryName}"   placeholder="">
            </div>
            <label class="col-md-1 control-label text-right" for="subCategoryName">刊登分类2：</label>
            <div class="col-md-3">
                <input class="form-control" id="subCategoryName" name="subCategoryName" type="text" value="${calculateDto.subCategoryName}"   placeholder="">
            </div>
        </div>
        <hr/>
        <div class="row">
            <label class="col-md-1 control-label text-right">title<font color="red">*</font></span>：</label>
            <div class="col-md-11">
        		<textarea class="form-control" name="title"
                          style="width: 600px; height: 100px; outline: none; resize: none">${product.title}</textarea>
            </div>
        </div>
        <hr/>
        <%--<div class="row">--%>
            <%--<label class="col-md-1 control-label text-right">description<font color="red">*</font></span>：</label>--%>
            <%--<div class="col-md-11">--%>
                ...
                <table class="table table-bordered"  id="supplier">
                    ...
                    <c:forEach items="${product.purchaseSourceList}" var="source" varStatus="s">
                        <tr>
                            ...
                            <td>${source.remark}</td>
                            <input type="hidden" class="changeSourceId" value="${source.sourceId}"/>
                            <input type="hidden" class="changeSourceName" value="${source.supplierName}"/>
            ...
            <div class="col-md-8">
                <table class="table table-bordered"  id="propertyTable">
                    ...
                    <tr id="propertyTr0">
                        <td align="center">
                            <div class="col-md-5">
                                <input class="form-control" id="property0" name="property0" type="text" value=""   placeholder="">
                            </div>
                        </td>
                        <td align="center">
                    ...
                    <c:forEach items="${propertyList}" var="property" varStatus="s">
                        <tr id="propertyTr${s.index}">
                            <td align="center">
                                <div class="col-md-5">
                                    <input class="form-control" id="property${s.index}" name="property${s.index}" readonly type="text" value="${property.propertyName}" placeholder=""/>
                                </div>
                            </td>
                            <td align="center">
                            ...
                            <td align="center">
                                <div class="row" style="margin-left: 10px;">
                                    <button class="btn btn-danger" type="button" onclick="del_property(${property.propertyCode})">删除</button>
                                </div>
                            </td>
                        </tr>
                    </c:forEach>
        ...
        <div class="row" style="margin-left: 50px;">
            <button class="btn btn-primary" id="mySave" type="button"  onclick="save(); return false;">保存</button>
            <button class="btn btn-primary" id="myDevelop" type="button" onclick="develop(); return false;">开发</button>
        </div>
