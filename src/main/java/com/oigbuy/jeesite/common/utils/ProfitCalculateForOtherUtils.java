 ...
 *
 */
public class ProfitCalculateForOtherUtils {

	private static double exchangeRate = 6.87; // 汇率(美元/人民币)
	private static double processingFee = 1; // 处理费
	private static double registeredFee = 0; // 挂号费

	private static double transFeePerGram = 0.085; // 运输的每克费用
	private static double transFeeDiscountRate = 0.9; // 运输费用的折扣率
	...
	public static double calDeclaredValue(double sellPrice){
		double declaredValue = 0;
		
		if(sellPrice >= 1 && sellPrice < 4){
			declaredValue = 1;
		}else if(sellPrice >= 4 && sellPrice < 7){
			declaredValue = 2;
		}else if(sellPrice >= 7 && sellPrice < 10){
			declaredValue = 4;
	...
	public static void main(String[] args) {
		double sellPrice = 3;
		double costPricae = 4.5;
		double weight = 100;
		
		System.out.println(ProfitCalculateForOtherUtils.calProfit(sellPrice, costPricae, weight));
		System.out.println(ProfitCalculateForOtherUtils.calProfitRate(sellPrice, costPricae, weight));
	}
