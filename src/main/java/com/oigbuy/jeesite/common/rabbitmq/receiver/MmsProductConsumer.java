 ...
 *
 */
public class MmsProductConsumer implements ChannelAwareMessageListener{
	...
	public void onMessage(Message msg, Channel channel) throws Exception {
		...
		try{
			...
			if(mmsParam != null){
				logger.info("mms产品开发成功或失败调用的接口参数："+new String(body));
				productMessureService.getMmsProduct(mmsParam);
			}
		}catch(Exception ex){
